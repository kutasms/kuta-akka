...
public class KSFObjectId {
	...
	static {
        ...
        pid = KSFByteUtil.intHex(process, 2);
        InetAddress addr;
        try {
			addr = InetAddress.getLocalHost();
			machine = KSFByteUtil.intHex(addr.getHostName().hashCode(), 2);
	...
	public static String generateId() {
		...
		current += INCREMENT;
		buffer.put(KSFByteUtil.longHex(current, 6));
		...
		buffer.put(seed);
		buffer.flip();
		return KSFByteUtil.bytesToHexString(buffer.array());
	}
	 ...
	 * */
	public static KSFObjectId parse(String id) throws DecoderException {
		return new KSFObjectId(id);
	}
	 ...
	 * */
	public KSFObjectId(String id) throws DecoderException {
		this.stringVal = id;
		...
		this.time = KSFByteUtil.bytesToLong(timeDst) - INCREMENT;
