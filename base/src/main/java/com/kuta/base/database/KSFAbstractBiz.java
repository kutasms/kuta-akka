 ...
 * */
public abstract class KSFAbstractBiz<T extends KSFDBEntity, TKey extends Number> {
	...
	public KSFAbstractBiz(String cacheKey) {
	 ...
	 * @throws KSFRuntimeException KSF运行时异常
	 * */
	protected String formatCacheKey(Object... args) throws KSFRuntimeException {
		if (!KSFUtil.isValueNull(args)) {
			String result = String.format(CACHE_KEY, args);
			return result;
		}
		throw new KSFRuntimeException(
				String.format(
						KSFError.CACHE_KEY_FORMAT_ERROR.getDesc(), args == null ? 
								KSFError.NOTHING_FOUND.getDesc() : 
	 ...
	 * @throws KSFRuntimeException KSF运行时异常
	 * */
	public int remove(SqlSession session,Jedis jedis, TKey key, Object... args) throws KSFRuntimeException {
	...
	public T getOneByKey(TKey key) throws Exception {
		String cacheKey = formatCacheKeyByTKey(key);
		return KSFRedisUtil.exec(jedis -> {
			return getOne(key, jedis, cacheKey);
		});
	}
	/**
	 ...
	 * @throws KSFRuntimeException KSF运行时异常
	 * */
	protected String formatCacheKeyByTKey(TKey key) throws KSFRuntimeException {
		String result = null;
		if(key.getClass().equals(Long.class) || long.class.equals(key.getClass())) {
			result = formatCacheKey(KSFUtil.longToBase64(key.longValue()));
		}
		result = formatCacheKey(KSFUtil.intToBase64(key.intValue()));
		return result;
	}
	...
	public String queryByKey(SqlSession session,Jedis jedis,String field, TKey key) throws Exception {
		...
		if(KSFUtil.isEmptyString(val)) {
			T t = get(session, key);
			if(KSFUtil.isValueNull(t)) {
				return null;
			}
	...
	public void cacheIncr(Jedis jedis, T t, Object... args) throws Exception {
		if (KSFUtil.isValueNull(t)) {
			throw new KSFIllegalArgumentException(
					String.format(KSFError.ARGUMENT_NOT_EMPTY.getDesc(), "实体对象"));
		}

		Field[] fields = KSFBeanUtil.getAllFields(t);
		...
		for (Field field : fields) {
			...
			if (KSFUtil.isValueNull(fieldObj)) {
				continue;
			}
	...
	public void cacheIncr(Jedis jedis, T t, TKey key) throws Exception {
		...
		if (!KSFUtil.isValueNull(key)) {
			if (fieldClazz.equals(Integer.class) || fieldClazz.equals(int.class)
				...
				cacheKey = formatCacheKey(KSFUtil.intToBase64(key.intValue()));
			} else {
				cacheKey = formatCacheKey(KSFUtil.longToBase64(key.longValue()));
			}
		}
		Pipeline pipe = jedis.pipelined();
		int n = 0;
