 ...
 * mongodb数据操作基类
 * */
public abstract class KSFMongoDao<T extends KSFMongoEntity> {
	...
	public Bson getFiltersByIdRange(Date start, Date end){
		...
		bufferStart.put(KSFByteUtil.longHex(start.getTime() + KSFObjectId.INCREMENT, 6));
		bufferStart.putInt(0);
		bufferStart.putShort((short)0);
		bufferStart.flip();
		String startHex = KSFByteUtil.bytesToHexString(bufferStart.array());
		ByteBuffer bufferEnd = ByteBuffer.allocate(12);
		bufferEnd.put(KSFByteUtil.longHex(end.getTime() + KSFObjectId.INCREMENT, 6));
		bufferEnd.putInt(0);
		bufferEnd.putShort((short)0);
		bufferEnd.flip();
		String endHex = KSFByteUtil.bytesToHexString(bufferEnd.array());
	...
	public T get(String id) {
		...
		if(KSFUtil.isValueNull(document)) {
			return null;
		}
